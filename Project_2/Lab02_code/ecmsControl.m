...
fuelFlwRate = zeros(length(w_speed), length(T_eng));
...
for i = 1: length(w_speed)
    ...
    for n = 1: length(T_eng)
         ...
         [SOC_next, fuelFlwRate(i, n), unfeas] = hev_model(SOC, [w_speed(i), T_eng(n)], [vehSpd, vehAcc], veh);
         ...
         if unfeas == 1
             ...
             fuelFlwRate_eq(i, n) = 10000;

         elseif SOC_next > 0.8 || SOC_next < 0.4 

             fuelFlwRate_eq(i, n) = 10000;

         else

            fuelFlwRate_eq(i, n) = fuelFlwRate(i, n) - eqFactor * const * (SOC_next-SOC)/dt;
...
[min_val, idx] = min(fuelFlwRate_eq(:));
